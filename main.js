...
window.onload = function(){
    alert("Xin chào Kim Ngân\n🙆‍♀️🙆‍♀️");
    
    alert("Tui là Vĩnh Kỳ\n👍👍");
    ...
    "use strict";
        ...
        countTimeOut = setTimeout(shoot,6500);
        //10 GIAY TU BAN
        }
        countTime();
        ...
        Arrow.prototype.drawArrow = function() {
            if(this.vis) {
                        ...
                        if(this.x < w-155){
                            ...
                            if(!(this.fy <= board.y-board.height/2 || this.fy >= board.y+board.height/2) || this.x > w){
                                if(this.x > w-110){
                                    ...
                                    score++;
                                    //console.log(score); cungdichuyen
                                    if(score === 3){
                                        arc.dy = 4;
                                    }
                                    else if(score === 5){
                ...
                try{
                                ...
                                const diem100 = ["gà quá đi", "dở thế", "hahaha","vui hông","tức hông"];
                                const diem120 = ["oke phết", "quá dữ", "hihihi","thua em tức quá","tức thiệt"];
                                var randome  = Math.floor(Math.random() * diem100.length);
                                if(bestScore <= 100)  alert( diem100[randome] + ' , không được 100 điểm😂') ;
                                else if (bestScore > 100 && bestScore <= 200) alert(diem120[randome] +' 😂');
                                else alert('ghê quá dị 😂');
